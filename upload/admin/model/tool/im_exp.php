...
class Modeltoolimexp extends Model {
	...
	public function load_data_in_db(){
		...
		if (move_uploaded_file($_FILES['csv']['tmp_name'], $uploadfile)) {
			if(file_exists($uploadfile)){
				...
				{	
					$query=$this->db->query("SELECT * FROM  " . DB_PREFIX . "product WHERE sku='".$data[$import_temp['product_sku']]."'");
					if(!$this->db->countAffected()){
						$data['product_model']=(!empty($import_temp['product_model']))?$data[$import_temp['product_model']]:"";
						$data['product_price']=(!empty($import_temp['product_price']))?$data[$import_temp['product_price']]:0;
						$data['product_description_name']=(!empty($import_temp['product_description_name']))?$data[$import_temp['product_description_name']]:"";
						$data['product_description_description']=(!empty($import_temp['product_description_description']))?$data[$import_temp['product_description_description']]:"";
						...
						$this->db->query("INSERT INTO `" . DB_PREFIX . "product` (`product_id`, `model`, `sku`, `upc`, `ean`, `jan`, `isbn`, `mpn`, `location`, `quantity`, `stock_status_id`, `image`, `manufacturer_id`, `shipping`, `price`, `points`, `tax_class_id`, `date_available`, `weight`, `weight_class_id`, `length`, `width`, `height`, `length_class_id`, `subtract`, `minimum`, `sort_order`, `status`, `date_added`, `date_modified`, `viewed`) VALUES (null,'".$data['product_model']."','".$data[$import_temp['product_sku']]."','','','','','','','".$data['product_quantity']."',".$data['product_stock_status_id'].",'data/import/".$data['product_image']."','".$data['product_manufacturer_id']."',1,'".$data['product_price']."',0,0,now(),0,1,0,0,0,1,1,1,999,".$data['product_status'].",now(),now(),0);");
						...
						$this->db->query("INSERT INTO `" . DB_PREFIX . "product_description` (`product_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`, `seo_title`, `seo_h1`, `tag`) VALUES (".$last.", 1, '".$data['product_description_name']."', '".$data['product_description_description']."', '".$data['product_description_meta_description']."', '".$data['product_description_meta_keyword']."', '".$data['product_description_seo_title']."', '".$data['product_description_seo_h1']."', '".$data['product_description_tag']."');");
						if(!empty($data['product_to_category_main_id'])){
							$this->db->query("INSERT INTO `" . DB_PREFIX . "product_to_category` (`product_id`, `category_id`, `main_category`) VALUES (".$last.", ".$data['product_to_category_main_id'].", 1);");
						}
						if(!empty($data['product_to_category_category_id_else'])){
							foreach ($data['product_to_category_category_id_else'] as $value) {
								// var_dump(array($value['category_id'], $data['product_to_category_main_id']));
	...
	public function createDB(){
		$this->db->query("CREATE TABLE `" . DB_PREFIX . "imports_tmpl` (	`id` INT(11) NOT NULL AUTO_INCREMENT,	`name` VARCHAR(255) NULL DEFAULT NULL,	`tpl` TEXT NOT NULL,	PRIMARY KEY (`id`) )");
	}
